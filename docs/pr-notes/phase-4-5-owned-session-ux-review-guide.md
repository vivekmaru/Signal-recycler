...
- `pnpm test`: passed. Workspace test run reported `packages/shared` no tests with `--passWithNoTests`, `apps/web` 4 files and 16 tests passed, and `apps/api` 17 files and 138 tests passed.
- `pnpm type-check`: passed. Workspace type-check completed for `packages/shared`, `apps/web`, and `apps/api`.
- `pnpm build`: passed. Workspace build completed for `packages/shared`, `apps/api`, and `apps/web`; Vite built 1718 modules and emitted `dist/index.html`, `dist/assets/index-CvF7CtH7.css`, and `dist/assets/index-D_a8Zefy.js`.
