...
- `apps/api/src/evals/suites/contextIndexEval.ts`
  ...
  - Converts temp-store setup and eval execution failures into suite failures so the eval report still renders.
- `apps/api/src/evals/suites/contextIndexEval.test.ts`
  ...
  - Covers non-5 retrieval limits, temp directory cleanup, temp-store setup failures, and index write failures.
