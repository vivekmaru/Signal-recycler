...
- `apps/api/src/evals/suites/contextIndexEval.ts`
  ...
  - Converts temp-store setup, eval execution, and store-close failures into suite failures so the eval report still renders.
- `apps/api/src/evals/suites/contextIndexEval.test.ts`
  ...
  - Covers non-5 retrieval limits, temp directory cleanup, temp-store setup failures, index write failures, and close failures.
