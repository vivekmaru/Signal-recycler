...
- Context envelope: centralizes memory retrieval, playbook injection, and memory usage audit events for owned-session adapters such as `mock` and `codex_cli`. The `codex_sdk` adapter retains the legacy proxy/mock compatibility path and should be treated as maintenance-only.
...
- Confirm owned-session adapters use the shared context envelope for retrieval, injection, and usage audit behavior. The `codex_sdk` compatibility adapter intentionally retains its legacy proxy/mock path for now.
...
- The SDK mock compatibility path still has legacy retrieval/injection code; this is documented as follow-up cleanup rather than expanded in Phase 4.

## Verification Commands And Results

- `pnpm --filter @signal-recycler/api test -- server.test.ts agentAdapters.test.ts contextEnvelope.test.ts codexCliAdapter.test.ts`: passed. API suite reported 17 files and 133 tests passed.
- `pnpm --filter @signal-recycler/api type-check`: passed.
- `pnpm test`: passed. Workspace tests completed; API suite reported 17 files and 133 tests passed. Shared and web packages had no test files and exited successfully with `--passWithNoTests`.
...
- Temporary API for memory smoke: started on `http://127.0.0.1:4318` with `PORT=4318`, `SIGNAL_RECYCLER_MOCK_CODEX=1`, and `SIGNAL_RECYCLER_DB=/private/tmp/signal-recycler-phase-4-smoke-rerun.sqlite`.
- `SIGNAL_RECYCLER_API_URL=http://127.0.0.1:4318 pnpm smoke:memory`: passed. Result: one package-manager memory selected and injected, zero candidate rules.
- Temporary API shutdown: completed; no process remained listening on port 4318.
