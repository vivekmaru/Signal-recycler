...
- Tests: adds server coverage for retaining a `command_convention` memory and asserting approved status plus `{ kind: "import", label: "api" }` source metadata.
...
- `pnpm --filter @signal-recycler/api test -- server.test.ts` - passed, 17 files / 131 tests.
