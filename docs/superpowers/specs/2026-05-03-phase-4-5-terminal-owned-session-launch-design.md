...
The first implementation should therefore support non-interactive terminal-owned sessions. It should use the same owned-session flow as dashboard runs and stream progress into both the terminal and dashboard.
...
The CLI should validate unsupported agents locally before calling `/api/sessions/:id/run` when possible, but the API remains authoritative.
...
- API returns 404 for session run: print session creation/run mismatch and exit non-zero.
